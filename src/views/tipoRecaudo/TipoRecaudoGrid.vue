  ...
  <div class="row d-flex justify-content-center">
    <div class="col-sm-11 col-md-11 col-lg-11 col-xl-11">
      <div class="card text-center shadow-lg p-3 mb-5 bg-white rounded">
        <div class="row">
          <div class="col-sm-11 col-md-11 col-lg-11 col-xl-11">
            <h1 class="display-6">Tipo Recaudo</h1>
          </div>
        ...
        <DxDataGrid
          ...
          @row-click="seleccionarTipoRecaudo"
          :showRowLines="true"
          :row-alternation-enabled="true"
        >
          <DxEditing :use-icons="true" mode="row"> </DxEditing>
          <DxSearchPanel :visible="true" :highlight-case-sensitive="true" />
          <template #tipoRecaudoCodigo>
            <b style="color: black">CÓDIGO</b>
          </template>
          <DxColumn data-field="codigo" header-cell-template="tipoRecaudoCodigo"/>
          <template #tipoRecaudoNombre>
            <b style="color: black">NOMBRE</b>
          </template>
          <DxColumn data-field="nombre" header-cell-template="tipoRecaudoNombre"/>
...
export default {
  ...
  setup(props, context) {
    ...
    const seleccionarTipoRecaudo = function (e) {
      store.commit("ocultarAlerta");
      context.emit("seleccionarTipoRecaudo", e.data.codigo);
    };
    ...
    const eliminar = function (rowData) {
      ...
      if (window.confirm("Desea eliminar este registro?")) {
        api
          .eliminarTipoRecaudo(rowData.row.values[0])
          .then(() => listar())
          .catch(() => {
            store.commit(
              "mostrarError",
